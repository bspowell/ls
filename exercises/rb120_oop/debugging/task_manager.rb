...
=begin
 on line 35, we originally had :
     tasks = tasks.select do |task|
We are initializing a local variable called tasks and then assign it to the return value of the next statement. 
However, the tasks.select doesn't work becuase tasks is now trying to refer to the local variable we just crreated
called 'tasks' which refers to nil. This doesn't do what we excpet which is to refer to the getter method 
and return an array. 
