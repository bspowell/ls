...
def computer_places_piece!(brd)
  ...
  #offense
  square = return_square(brd, COMPUTER_MARKER)
  if square 
  ...
  #defense
  square = return_square(brd, PLAYER_MARKER)
  if square 
    brd[square] = COMPUTER_MARKER
    return
  end
...
def choose_starting_player
  prompt "Player (1) or Computer (2) to go first?"
  answer = gets.chomp.to_i
  answer
end

def starting_loop_player(board)
  loop do
    display_board(board)
...
def starting_loop_computer(board)
  loop do
    computer_places_piece!(board)
    break if someone_won?(board) || board_full?(board)
...
loop do
  ...
  loop do
    ...
    if choose_starting_player == 1
      starting_loop_player(board)
    else
      starting_loop_computer(board)
    end
