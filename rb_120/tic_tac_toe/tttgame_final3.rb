...
class Board
  ...
  def count_human_marker(squares) #passing in an array of square objects like #<Square:0x000000011414d038 @marker=" ">
    squares.collect(&:marker).count(human.marker)
  end
...
class TTTGame
  ...
  attr_accessor :turn, :human_score, :computer_score, :human_marker, :initial_markers
  ...
  def pick_marker
    loop do
      puts "First, pick a marker (SINGLE LETTER)"
      human.marker = gets.chomp
      break if human.marker.length == 1
      puts "That's not a single letter"
    end
  end
  # def pick_name
  #   puts "Now, pick a name"
  #   @name = gets.chomp
  # end
  ...
  def reset
    board.reset
    turn = [human.marker, COMPUTER_MARKER].sample
    clear_screen
  end
